...
public:
    ...
    int getPosOf(int id)const{

        if(headPtr!=nullptr && itemCount<99999){
            Node<ItemType>* curPtr=headPtr;
            int i=1;
            while(curPtr!=nullptr){
                //std::cout<<"*"<<curPtr->getNodePtr()->getId()<<"*";
                if(curPtr->getNodePtr()!=nullptr  && curPtr->getNodePtr()->getId()==id){
                    //std::cout<<"3";
                    return i;
                }
                i++;
    ...
    std::string toString(){
        ...
        Node<ItemType>* nodePtr = headPtr;
        str+= "["+std::to_string(nodePtr->getItem().getId());
